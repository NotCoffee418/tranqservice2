...
class DaemonMain {
  static void runDaemon() async {
    ...
    LogAccess.addLog(LogVerbosity.debug, 'Daemon Mode: Starting up...');

    await DatabaseService.getDb(); // Ensure database is initialized
    await YtdlpService.ensureYtdlp(); // Update yt-dlp if needed

    LogAccess.addLog(LogVerbosity.debug, 'Daemon Mode: Running background tasks...');

    while (true) {
      try {
        await _runDaemonIteration();
      } catch (e) {
        LogAccess.addLog(LogVerbosity.error, 'Daemon encountered an error: \$e');
      }
  ...
  static Future<void> _runDaemonIteration() async {
    LogAccess.addLog(LogVerbosity.info, 'Checking for new videos...');
    ...
    for (final playlist in playlists) {
      ...
      if (info == null) {
        LogAccess.addLog(LogVerbosity.error, 'Failed to fetch info for playlist: \$url');
        continue;
      }
      ...
      LogAccess.addLog(LogVerbosity.debug, 'Playlist "${info["title"]}" contains $videoCount videos.');
